...
/***
 * Round robin task scheduler (systick handler lol)
 */
...
void init_systick_timer(uint32_t tick_hz)
{
	...
	*pSRVR &= ~(0x00FFFFFF);
	//Load the value into SVR (Reload register)
	*pSRVR |= count_value;

	*pSCSR |= (1 << 1); //Enable the systick timer
	*pSCSR |= (1 << 2);
	*pSCSR |= (1 << 1);
...
void init_tasks_stack()
{
	...
	for(int i =0; i < MAX_TASKS; i++)
	{
		...
		pPSP--;
		*pPSP = DUMMY_XPSR; //SHOULD BE CODED TO 0X0100000
...
__attribute__((naked)) void switch_sp_to_psp(void)
{
	//Initialize PSp with task1 stack start addr
	//Get the value of Current task
	__asm volatile("PUSH {LR}");	//Preserve LR which connects back to main (LR corrupted otherwise)
